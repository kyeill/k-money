...
dark crest is a white cockerel, which is what it should be. That file is
generated by sports-daily's `logos.py --write`, which measures the actual
pixels of both variants, and copied here so the two pages show the same crest.
...
python selftest.py         434 assertions, no key and no network needed
```
