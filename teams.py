...
def logo_overrides():
    ...
    pixels of both variants, and copied here so the two pages show the same
    crest. Re-copy it when that list is regenerated.
    """
    global _logo_overrides
    if _logo_overrides is None:
        try:
            with open(LOGO_OVERRIDES_FILE, encoding="utf-8") as fh:
                _logo_overrides = json.load(fh)
        except Exception:
            _logo_overrides = {}
